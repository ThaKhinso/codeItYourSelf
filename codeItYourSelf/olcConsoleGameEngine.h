...
keycode you want to know about. bPressed is set for the frame the key is pressed
down in, bHeld is set if the key is held down, bReleased is set for the frame
the key is released in. The same applies to mouse! m_mousePosX and Y can be used
to get the current cursor position, and m_mouse[1..5] returns the mouse buttons.
...
public:
  ...
  int ConstructConsole(int width, int height, int fontw, int fonth) {
    ...
    // Unfortunately, MSDN does not offer much by way of useful information, and
    // so the resulting sequence is the reult of experiment that seems to work
    // in multiple cases.
    //
    ...
    m_rectWindow = {0, 0, (short)m_nScreenWidth - 1,
                    (short)m_nScreenHeight - 1};
    if (!SetConsoleWindowInfo(m_hConsole, TRUE, &m_rectWindow))
      return Error(L"SetConsoleWindowInfo");

    // Set flags to allow mouse input
    if (!SetConsoleMode(m_hConsoleIn, ENABLE_EXTENDED_FLAGS |
                                          ENABLE_WINDOW_INPUT |
                                          ENABLE_MOUSE_INPUT))
  ...
  void Fill(int x1, int y1, int x2, int y2, short c = 0x2588,
            short col = 0x000F) {
    Clip(x1, y1);
    Clip(x2, y2);
    for (int x = x1; x < x2; x++)
  ...
  void DrawLine(int x1, int y1, int x2, int y2, short c = 0x2588,
                short col = 0x000F) {
...
protected
  ...
  public:
    ...
    olcAudioSample(std::wstring sWavFile) {
      ...
      std::fread(&dump, sizeof(char), 4, f); // Not Interested
      std::fread(&wavHeader, sizeof(WAVEFORMATEX) - 2, 1,
                 f); // Read Wave Format Structure chunk
                     // Note the -2, because the structure has 2 bytes to
  ...
  bool CreateAudio(unsigned int nSampleRate = 44100, unsigned int nChannels = 1,
    ...
    if (waveOutOpen(&m_hwDevice, WAVE_MAPPER, &waveFormat,
                    (DWORD_PTR)waveOutProcWrap, (DWORD_PTR)this,
    ...
    for (unsigned int n = 0; n < m_nBlockCount; n++) {
      ...
      m_pWaveHeaders[n].lpData =
          (LPSTR)(m_pBlockMemory + (n * m_nBlockSamples));
    }
  ...
  void waveOutProc(HWAVEOUT hWaveOut, UINT uMsg, DWORD dwParam1,
                   DWORD dwParam2) {
    if (uMsg != WOM_DONE)
      return;
    m_nBlockFree++;
  ...
  static void CALLBACK waveOutProcWrap(HWAVEOUT hWaveOut, UINT uMsg,
                                       DWORD dwInstance, DWORD dwParam1,
                                       DWORD dwParam2) {
    ((olcConsoleGameEngine *)dwInstance)
  ...
  void AudioThread() {
    ...
    while (m_bAudioThreadActive) {
      ...
      waveOutPrepareHeader(m_hwDevice, &m_pWaveHeaders[m_nBlockCurrent],
                           sizeof(WAVEHDR));
      waveOutWrite(m_hwDevice, &m_pWaveHeaders[m_nBlockCurrent],
                   sizeof(WAVEHDR));
      m_nBlockCurrent++;
  ...
  // just playing audio clips (think a synthesizer for example) in whcih case we
  ...
  float GetMixerOutput(int nChannel, float fGlobalTime, float fTimeStep) {
    ...
    for (auto &s : listActiveSamples) {
      ...
      s.nSamplePosition += (long)((float)vecAudioSamples[s.nAudioSampleID - 1]
...
protected:
  ...
  // These need to be static because of the OnDestroy call the OS may make. The
  // OS spawns a special thread just for that
